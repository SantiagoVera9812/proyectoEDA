...
void crear_mapa(std::list<int*>* newLista, int nVecinos, int nElementos) {
    ...
    for (int* i : *newLista) {
        coords[0] = i[0];
        coords[1] = i[1];
        KdTree.insertBalTree(coords, index);
        index++;
    }
    
    Graph graph(nElementos);

    int rootCoords[2];
    rootCoords[0] = KdTree.getRoot()->point[0];
    rootCoords[1] = KdTree.getRoot()->point[1];
    KdTree.preorder();
    std::list<Node<2>*> listaVecinos = KdTree.vecinosMasCercanos(rootCoords, nVecinos);
    for (Node<2>* node : listaVecinos) {
        ...
        std::cout << "Coordenadas: (" << node->point[0] << ", " << node->point[1] << ")\n";
    }

    Node<2>* secondNearest = nullptr;
    getNearestNeighbor(KdTree.getRoot(), listaVecinos, secondNearest);
    if (secondNearest != nullptr) {
        std::cout << "Coordenadas del vecino más cercano: (" << secondNearest->point[0]
                  << ", " << secondNearest->point[1] << ")\n";
        std::cout << "distancia: " << secondNearest->distancia << std::endl;
    } else {
        std::cout << "No se encontró un segundo vecino más cercano.\n";
    }
     std::cout << "Coordenadas raiz: (" << KdTree.getRoot()->point[0]
                  << ", " << KdTree.getRoot()->point[1] << ")\n";
        std::cout << "distancia: " << secondNearest->distancia << std::endl;
    double rootDistance = secondNearest->distancia;
    double DistanciaInicial = rootDistance;
    std::pair<int, int> currentPoint = std::make_pair(KdTree.getRoot()->point[0], KdTree.getRoot()->point[1]);
       std::pair<int, int> neighborPoint = std::make_pair(secondNearest->point[0], secondNearest->point[1]);
    graph.addEdge(currentPoint, neighborPoint, rootDistance);
    std::unordered_set<Node<2>*> visited;
    visited.insert(KdTree.getRoot());
    // Recursively find the nearest neighbors for the root neighbor
    findNearestNeighborsRecursively(KdTree, secondNearest, nVecinos - 1, graph, visited, DistanciaInicial, index);
...
void ruta_mas_larga(std::list<int*>* newLista, int nElementos){
  ...
  std::pair<Node<2>*, Node<2>*> NodosMinMax;

  Node<2>* leftmost = nullptr;
   std::list<Node<2>*> nodeList;
    for (int* i : *newLista) {
        Node<2>* node = KdTree.search(i);
        if (node != nullptr)
            nodeList.push_back(node);
    }
    getNearestNeighbor(KdTree.getRoot(), nodeList, leftmost);

  Node<2>* rightmost = nullptr;
    std::list<Node<2>*> nodeListRight;
  for (int* i : *newLista) {
        Node<2>* node2 = KdTree.search(i);
        if (node2 != nullptr)
            nodeListRight.push_back(node2);
    }
  std::list<Node<2>*> VeciLista = KdTree.vecinosMasCercanos(rootCoords, nElementos);
  rutaMasLarga(KdTree.getRoot(), VeciLista, rightmost);

  NodosMinMax.first = leftmost;
  NodosMinMax.second = rightmost;
  if (NodosMinMax.first != nullptr) {
    std::cout << "Punto minimo: (" << NodosMinMax.first->point[0] << ", " << NodosMinMax.first->point[1] << ")" << std::endl;
} else {
    std::cout << "Nodo Minimo no encontrado." << std::endl;
}

if (NodosMinMax.second != nullptr) {
    std::cout << "Punto Maximo: (" << NodosMinMax.second->point[0] << ", " << NodosMinMax.second->point[1] << ")" << std::endl;
} else {
    std::cout << "PuntoMaximo no encontrado." << std::endl;
}
std::list<int*> creaNuevaLista = KdTree.preorderList();
for(int* i : creaNuevaLista){
std::cout << "Coordenadas de los vecinos: (" << i[0]
                  << ", " << i[1] << ")\n";
}
/*index = 0;
KdTree.clear();
KdTree.setRoot(rightmost);
        int Newrootcoords[2];
  int NewCoords[2];
    Newrootcoords[0] = KdTree.getRoot()->point[0];
    Newrootcoords[1] = KdTree.getRoot()->point[1];
index = 0;
  for (int* i : creaNuevaLista) {
        
    if(i!= nullptr){
        if (isValidIntArray(i))
        {
        int NuevasCoord[2];
        NuevasCoord[0] = i[0];
        NuevasCoord[1] = i[1];
       std::cout << "Coordenadas de los vecinos: (" << i[0]
                  << ", " << i[1] << ")\n";
        KdTree.insert(NuevasCoord, index);
        index++;
        }
  }
    }*/
  
std::list<Node<2>*> vecinosAPasar;
  for (int* i : creaNuevaLista) {
        Node<2>* node3 = KdTree.search(i);
        if (node3 != nullptr)
            vecinosAPasar.push_back(node3);
    }
//std::list<Node<2>*> inicializar = KdTree.vecinosMasCercanos(rootCoords, nElementos);

  Node<2>* secondNearest = nullptr;
    getNearestNeighbor(KdTree.getRoot(), vecinosAPasar, secondNearest);
 ...
 double rootDistance = secondNearest->distancia;
  ...
  std::cout<<"Nel"<<std::endl;
   PuntoAOtro(KdTree, secondNearest, nElementos - 1, graph, visited, DistanciaInicial, index, leftmost);
