...
//agregar_elemento userInput[1], tipo_comp userInput[2], tamaño userInput[3], unidad_med[4], coordX[5], coordY[6]
   ...
   func["crear_mapa"] = [&argc,&userInput, &KDTree,&listaDeElementos](){
     ...
     if((argc - 1) != 2){
       ...
       printf("Comando invalido\n");
        ...
        try{
         float factor_conectividad = std::stof(userInput[2].c_str());
         
         if(factor_conectividad <= 1){
           int nElementos = countStructs(listaDeElementos);
           int VecinosAEncontrar = factor_conectividad*nElementos;
           ubicar_elementos(&listaDeElementos, &KDTree);
           std::list<int*> newLista = KDTree.preorderList();
           crear_mapa(&newLista,VecinosAEncontrar, nElementos);
         }else{
           printf("Comando invalido\n");
         }
    ...
    func["ruta_mas_larga"] = [&argc,&userInput, &KDTree,&listaDeElementos](){

      if((argc-1) != 1){
            printf("Comando invalido\n");
        }else{
        int nElementos = countStructs(listaDeElementos);
        std::cout<<nElementos<<std::endl;
        ubicar_elementos(&listaDeElementos, &KDTree);
        std::list<int*> newLista = KDTree.preorderList();
        ruta_mas_larga(&newLista, nElementos);
        }
